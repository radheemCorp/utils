...
alias kextsvc='kubectl get svc -A -o custom-columns="NAMESPACE:.metadata.namespace,NAME:.metadata.name,EXTERNAL-DNS:.metadata.annotations.external-dns\.alpha\.kubernetes\.io/hostname,LB-IP:.status.loadBalancer.ingress[*].ip,LB-HOSTNAME:.status.loadBalancer.ingress[*].hostname"'
alias kimg='kubectl get deployments -A -o custom-columns="NAMESPACE:.metadata.namespace,NAME:.metadata.name,IMAGE:.spec.template.spec.containers[*].image"'
alias ka='kubectl get -A'
alias kl='kubectl -n local'
